...
class Relation(Node):
    def __init__(self, name, query_block_idx=0, nesting_level=0):
        super().__init__(
            name, query_block_idx=query_block_idx, nesting_level=nesting_level
        )
        self.abbre = "R"
...
class Function(Node):
    def __init__(self, aggregation_type, query_block_idx=0, nesting_level=0):
        super().__init__(
            aggregationTypeToName[aggregation_type],
            query_block_idx=query_block_idx,
...
class Query_graph(nx.DiGraph):
    ...
    def root(self):
        def number_of_proj_att(node):
            return len(
                [
                    1
                    for src, dst in self.edges(node)
                    if type(self.edges[src, dst]["data"]) == Projection
                ]
            )

        # Extract level 0, block 0
        outer_block_relations = filter(
            lambda k: k.query_block_idx == 0
            and k.nesting_level == 0
            and type(k) == Relation,
            self.nodes._nodes,
        )
        relations_sorted_by_proj_att = sorted(
            outer_block_relations, key=lambda k: -number_of_proj_att(k)
        )
    ...
    def set_join_directions(self, given_node=None):
        ...
        def is_for_join(n1, edge, n2):
            ...
            return type(edge["data"]) == Join or (
                type(edge["data"]) == Operation
                and type(n1) == Attribute
                and type(n2) == Attribute
            )
        ...
        for neighbor in self.neighbors(given_node):
            ...
            if (
                in_edge
                and is_for_join(neighbor, in_edge, given_node)
                and is_for_join(given_node, out_edge, neighbor)
            ):
    ...
    def connect_selection(self, src_node, dst_node, operation_idx=0):
        self.unidirectional_connect(
            src_node, Selection(src_node, dst_node, operation_idx), dst_node
        )

    def connect_join(self, relation1, r1_attribute, r2_attribute, relation2):
        self.bidirectional_connect(
            relation1, Join(relation1, r1_attribute), r1_attribute
        )
        self.bidirectional_connect(
            relation2, Join(relation2, r2_attribute), r2_attribute
        )
        self.bidirectional_connect(
            r1_attribute,
            Operation(OperatorType.Equal, r1_attribute, r2_attribute),
            r2_attribute,
        )

    def connect_operation(self, relation, operator_type, attribute):
        self.unidirectional_connect(
            relation, Operation(operator_type, relation, attribute), attribute
        )

    def connect_aggregation(self, attribute, aggregation_func):
        self.unidirectional_connect(
            attribute, Aggregation(attribute, aggregation_func), aggregation_func
        )
    ...
    def connect_having(self, relation, attribute, operation_idx=0):
        self.unidirectional_connect(
            relation, Having(relation, attribute, operation_idx), attribute
        )
    ...
    def connect_order(self, relation, attribute, is_asc=True):
        self.unidirectional_connect(
            relation,
            Ordering(
                OrderingType.Ascending if is_asc else OrderingType.Descending,
                relation,
                attribute,
            ),
            attribute,
